<div class="page-heading"><div><p class="eyebrow">SPOTS</p><h1>スポット管理</h1></div><div class="export-actions"><?php if ($event !== null): ?><a class="button secondary" href="<?= htmlspecialchars($url('admin/print/entrance'), ENT_QUOTES, 'UTF-8') ?>">入口案内</a><?php endif; ?><?php if ($spots !== []): ?><a class="button secondary" href="<?= htmlspecialchars($url('admin/print/spots'), ENT_QUOTES, 'UTF-8') ?>">全スポット印刷</a><a class="button secondary" href="<?= htmlspecialchars($url('admin/print/spots.pdf'), ENT_QUOTES, 'UTF-8') ?>">PDF</a><?php endif; ?><a class="button" href="<?= htmlspecialchars($url('admin/spots/create'), ENT_QUOTES, 'UTF-8') ?>">スポットを追加</a></div></div>
...
<article class="panel spot-card <?= $spot['is_active'] ? '' : 'spot-inactive' ?>">
    ...
    <div class="spot-actions">
        ...
        <a class="button small secondary" href="<?= htmlspecialchars($url("admin/spots/{$spot['management_token']}/print"), ENT_QUOTES, 'UTF-8') ?>">印刷</a>
